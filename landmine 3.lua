...
--add users home planets to the map
	--for j=1,numRings do
		a = math.random(0,360)
		--planetRadius = 24
		radius = radius-(planetRadius*4)-200
		circum = 2*math.pi*radius
		local numPlanets = circum/(planetRadius*4)
		if(numPlanets%1 ~= 0) then
			local dec = numPlanets%1
			local overRad = (dec*4)/circum
			--print(overRad)

		--	planetRadius = planetRadius+overRad
		end
		--planetRadius = (circum/(numPlanets*4))
		local totalPlanets = 200
		radius = 100
		
    for i=1,totalPlanets do
        local x,y
        local circum = 2*math.pi*radius
        numPlanets = circum/(planetRadius*4) 
        print(numPlanets)
        radius = radius+(1/numPlanets)*200
        x = sw/2 + radius*math.cos(a*math.pi/180.0)/2.0
        y = sh/2 + radius*math.sin(a*math.pi/180.0)/2.0
        local prod = 100
   ...
   local finalPlanet = g2.new_planet(neutral, sw/2, sh/2, 200, 0)
