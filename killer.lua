...
function init_killer()
---------------------------------------------------------------------------
--ADD THE MODE
---------------------------------------------------------------------------
local mode = {}
GAME.mode:addMode(mode,"killer")
mode.description = "Kill the plague!"
mode.presets = {}
...
mode.init = function(users,neutral)
    ...
    if(#users > 0) then
    	local ind = math.random(1,#users)
   		data.enemy = users[ind]
   		--users[ind] = nil
   	end
   	data.enemy.render_color = 0x777777
   	--data.enemy.ui_to_mask = 0x000000

 for i,user in pairs(users) do
 	if(user.title_value == "monsterV") then
 		--data.enemy = user
 	end
 end
  ...
  --  data.enemy.user_uid = 10000000
   -- users[#users+1] = data.enemy
    ...
    local p = g2.new_planet(data.enemy,sw/2,sh/2,100,(numUsers-1)*105)
     p.has_motion = true
     p.has_physics = true
    -- p.motion_vx = MAX_SPEED*(2*math.random() - 1)*math.cos(p.position_x*p.position_y)
      --  p.motion_vy = MAX_SPEED*(2*math.random() - 1)*math.sin(p.position_x*p.position_y)
...
mode.loop = function(t)
	...
	local planets = g2.search("planet -neutral")
 	local winner = nil
  		...
  		local enemyPlanets = g2.search("planet owner:"..data.enemy)
		local otherPlanets = g2.search("planet -owner:"..data.enemy)
		local numEnemyPlanets = 0
		local numOtherPlanets = 0
		for i,planet in pairs(enemyPlanets) do
			numEnemyPlanets = numEnemyPlanets + 1
		end
		for i,planet in pairs(otherPlanets) do
			if(planet:owner() ~= GAME.game.neutral) then
				numOtherPlanets = numEnemyPlanets + 1
			end
		end
		if(numOtherPlanets == 0) then
			winner = data.enemy
		end
		if(numEnemyPlanets == 0) then
			winner = data.enemy
		end
		
      	if (winner == nil) then data.winT = data.winTime return end
